import { logoutSession } from '../../services/cloudflare';
...
const UserDashboard = () => {
  ...
  const handleDownloadCertificate = async (certificate) => {
    try {
      ...
      if (certificate.missingFile || !certificate.downloadUrl) throw new Error('File perlu diunggah ulang oleh admin');
      const response = await window.fetch(certificate.downloadUrl, { credentials: 'include' });
      if (!response.ok) throw new Error('File tidak tersedia atau sesi login berakhir');
      const href = URL.createObjectURL(await response.blob());
      const link = document.createElement('a');
      link.href = href; link.download = certificate.originalName || 'certificate.pdf';
      document.body.appendChild(link); link.click(); link.remove();
      setTimeout(() => URL.revokeObjectURL(href), 10000);
      setUserStats(prev => ({ ...prev, totalDownloads: prev.totalDownloads + 1 }));
